...
void addRecord(Gradebook* gradebook, int studentId, double grade)
{
	...
	GradeRecord* pNewRecords = malloc(newNumRecords *
		sizeof(GradeRecord));
	...
	GradeRecord* newRecord = &(gradebook->pRecords[gradebook->numRecords
		- 1]);
...
int main()
{
	...
	printGrades(grades);
	// TODO cleanup the gradebook along with its current graderecords
	// HINT: everything that has been allocated on the heap needs to be	freed
	// NOTE TO SELF: malloc() runs for Gradebook initializer and GradeRecord pseudo-initializer
	free(grades->pRecords); // Similar to the free() command in addRecord(). I imagine this should work roughly the same way.
	printGrades(grades);
	free(grades);
	//printGrades() shouldn't work at this point.
	//printGrades(grades);
	//woot! printGrades() crashes the program.
}